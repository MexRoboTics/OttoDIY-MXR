...
void Otto::init(byte YL, byte YR, byte RL, byte RR, bool load_calibration, byte NoiseSensor, byte Buzzer, byte USTrigger, byte USEcho) {
  ...
  for (byte i = 0; i < 4; i++) servo_position[i] = 90;
...
void Otto::_moveServos(int time, int  servo_target[]) {
  ...
  if(time>10){
    for (byte i = 0; i < 4; i++) increment[i] = ((servo_target[i]) - servo_position[i]) / (time / 10.0);
    final_time =  millis() + time;

    for (byte iteration = 1; millis() < final_time; iteration++) {
      partial_time = millis() + 10;
      for (byte i = 0; i < 4; i++) servo[i].SetPosition(servo_position[i] + (iteration * increment[i]));
      while (millis() < partial_time); //pause
    }
  }
  else{
    for (byte i = 0; i < 4; i++) servo[i].SetPosition(servo_target[i]);
  }
  for (byte i = 0; i < 4; i++) servo_position[i] = servo_target[i];
}
...
//---------------------------------------------------------
//-- Otto getNoised: return Otto's noise digital sensor measure
//---------------------------------------------------------
int Otto::getNoised(){
  ...
  byte numReadings = 5;  
    ...
    //noiseLevel = analogRead(pinNoiseSensor); 
	noiseLevel = !digitalRead(pinNoiseSensor);  // Mi sensor es digital
	

    for(byte i=0; i<numReadings; i++){
        //noiseReadings += analogRead(pinNoiseSensor);
		noiseReadings += !digitalRead(pinNoiseSensor);
        delay(4); // delay in between reads for stability
    }
